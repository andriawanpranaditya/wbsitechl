...
export default function ProjectGrid({ filters }: { filters: Filters }) {
  const list = filterProjects(filters); const t = getDict();
  if (list.length === 0) return (
    <div className="card p-10 text-center"><p className="font-display text-xl text-forest">{t.finder.empty}</p><p className="mt-2 text-sm text-stone">{t.finder.emptyHint}</p></div>
  );
