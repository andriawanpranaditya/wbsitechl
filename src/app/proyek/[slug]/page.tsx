...
const sections = [['overview','Overview'],['location','Lokasi'],['concept','Konsep'],['product','Produk'],['facilities','Fasilitas'],['siteplan','Site plan'],['gallery','Galeri'],['advantages','Keunggulan lokasi'],['price','Harga & promo'],['faq','FAQ'],['contact','Kontak']];
...
export default function ProjectPage({ params }: { params: { slug: string } }) {
  ...
  const wa = waLink(p.sales.whatsapp, `Halo, saya tertarik dengan ${p.name}. Boleh minta info & price list?`);
  const schema = {
    ...
    <article style={{ ['--accent' as string]: p.accent }}>
      ...
      <section id="overview" className="relative scroll-mt-32 bg-forest-deep text-ivory">
        ...
        <div className="container-site relative pb-16 pt-28 md:pb-24 md:pt-40">
          <nav aria-label="breadcrumb" className="text-xs text-ivory/70"><Link href="/">Home</Link> / <Link href={`/development/${p.type}`}>{typeLabel[p.type]}</Link> / {p.name}</nav>
          <p className="eyebrow mt-6 !text-gold">{p.status} · {p.location}</p>
          <h1 className="h-display mt-3 text-5xl md:text-7xl">{p.name}</h1>
          <p className="mt-3 font-display text-2xl italic text-ivory/90">{p.tagline}</p>
          <div className="mt-8 flex flex-wrap items-center gap-4">
            <div><p className="text-xs text-ivory/70">Harga mulai</p><p className="font-display text-3xl">{rupiah(p.priceFrom)}</p></div>
            <a href={wa} target="_blank" rel="noreferrer" data-track="wa_click" className="btn-gold">Chat sales via WhatsApp</a>
            <a href="#price" className="btn border border-ivory/30 text-ivory hover:bg-ivory/10">Unduh price list</a>
          </div>
        </div>
      </section>
      ...
      <div className="container-site grid gap-16 py-16 lg:grid-cols-[1fr_360px]">
        <div className="space-y-20">
          <section>
            <p className="text-lg leading-relaxed text-ink/90 md:text-xl">{p.overview}</p>
          ...
          {/* 2. Location */}
          <section><H id="location" title="Lokasi" />
            <p className="mt-3 text-stone">{p.location}</p>
            <div className="mt-6 overflow-hidden rounded-2xl border border-sand">
              <iframe title={`Peta ${p.name}`} loading="lazy" className="h-[360px] w-full" src={`https://maps.google.com/maps?q=${p.coordinates.lat},${p.coordinates.lng}&z=14&output=embed`} />
            </div>
          </section>

          {/* 3. Concept */}
          <section><H id="concept" title="Konsep" /><p className="mt-4 text-lg leading-relaxed">{p.concept}</p></section>
              ...
              {p.units.map((u) => (
                <div key={u.name} className="card p-6">
                  ...
                  <dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
                    ...
                    <div><dt className="text-stone">LB</dt><dd className="font-semibold">{u.buildingArea} m²</dd></div>
                    {u.bedrooms > 0 && <div><dt className="text-stone">K. tidur</dt><dd className="font-semibold">{u.bedrooms}</dd></div>}
                    <div><dt className="text-stone">K. mandi</dt><dd className="font-semibold">{u.bathrooms}</dd></div>
                  </dl>
          ...
          {/* 8. Advantages */}
          <section><H id="advantages" title="Keunggulan lokasi" />
          ...
          {/* 9. Price & promo */}
          <section><H id="price" title="Harga & promo" />
            {projectPromos.length > 0 ? projectPromos.map((pr) => (
              <div key={pr.slug} className="mt-6 rounded-2xl p-6 text-white" style={{ background: p.accent }}>
                <p className="text-xs uppercase tracking-wider opacity-80">Berlaku s.d. {formatDate(pr.endDate)}</p>
                <p className="mt-2 font-display text-2xl">{pr.title}</p><p className="mt-2 text-sm opacity-90">{pr.summary}</p>
              </div>
            )) : <p className="mt-4 text-stone">Hubungi sales kami untuk penawaran terbaik bulan ini.</p>}
            <div className="card mt-6 p-6">
              <p className="font-display text-xl text-forest">Unduh price list terbaru</p>
              <p className="mt-1 text-sm text-stone">Isi data singkat, price list PDF akan dikirim ke WhatsApp Anda.</p>
              <div className="mt-5"><LeadForm project={p.slug} source="pricelist" compact cta="Kirim price list" /></div>
            </div>
          </section>

          {/* 10. FAQ */}
          <section><H id="faq" title="Pertanyaan umum" />
        ...
        <aside className="lg:sticky lg:top-32 lg:self-start">
          <div id="contact" className="card scroll-mt-32 p-6">
            <p className="eyebrow">Hubungi {p.sales.name}</p>
