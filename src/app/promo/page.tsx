...
export default function PromoPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Promo & event" title="Penawaran dan agenda terbaru" lead="Promo berlaku selama periode tertera. Syarat & ketentuan berlaku." />
      <div className="container-site mt-12 grid gap-6 md:grid-cols-2">
        {sorted.map((pr) => { const active = isActive(pr.endDate); const proj = pr.projectSlug ? getProject(pr.projectSlug) : undefined; return (
          <div key={pr.slug} id={pr.slug} className={`card scroll-mt-28 overflow-hidden md:grid md:grid-cols-[220px_1fr] ${active ? '' : 'opacity-60'}`}>
            ...
            <div className="p-6">
              <div className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${active ? 'bg-gold text-forest-deep' : 'bg-sand text-stone'}`}>{active ? 'Berlangsung' : 'Selesai'}</span><span className="eyebrow">{pr.kind}</span></div>
              ...
              {proj && <Link href={`/proyek/${proj.slug}#price`} className="mt-4 inline-block text-sm font-semibold text-gold-deep hover:underline">Lihat {proj.name} →</Link>}
            </div>
          </div>); })}
      </div>
    </>
