    ...
    <>
      ...
      <div className="container-site mt-16 grid grid-cols-2 gap-6 md:mt-24 md:grid-cols-4">
        {stats.map((s, i) => (
          <Reveal key={s.label} delay={i * 100} className="border-l-2 border-gold pl-4">
            ...
            <p className="mt-1 text-sm text-stone">{t.company.stats[i] ?? s.label}</p>
          </Reveal>
        ))}
      </div>

      {/* Featured projects */}
      <Section eyebrow={t.home.featuredEyebrow} title={t.home.featuredTitle} lead={t.home.featuredLead}>
        ...
        <div className="mt-8 text-center"><Link href="/development" className="btn-ghost">{t.common.viewAll}</Link></div>
      </Section>

      {/* Promo */}
      {activePromos.length > 0 && (
        <Section eyebrow={t.home.promoEyebrow} title={t.home.promoTitle} className="bg-white">
      ...
      <Section eyebrow={t.home.trustEyebrow} title={t.home.trustTitle}>
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div>
            <p className="text-sm text-stone">{t.home.banks}</p>
            ...
            <Link href="/kalkulator-kpr" className="btn-ghost mt-6">{t.home.kpr}</Link>
          </div>
        </div>
      </Section>

      {/* News */}
      <Section eyebrow={t.home.newsEyebrow} title={t.home.newsTitle} className="bg-white">
        <div className="grid gap-6 md:grid-cols-3">
